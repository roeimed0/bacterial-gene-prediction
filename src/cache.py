...
def add_genome_to_cache(
    genome_id: str, email: str, cached_data: Dict, min_length: int = None
) -> bool:
    ...
    cached_data[genome_id] = {"sequence": genome_sequence, "orfs": orfs}

    print(f"[DONE] Added {genome_id} to cache")
    return True


def precompute_genomes(
    genome_ids: List[str],
