...
def plot_score_distributions(all_orfs: List[Dict], reference_genes: Set, genome_id: str = None):
    ...
    plt.suptitle(
        f'Unnormalized Score Distributions - TP vs FP\n{genome_id or ""}',
        fontsize=14,
        y=1.00,
    )
    plt.tight_layout()
    ...
    if "combined_score" in all_orfs[0]:
        ...
        if tp_combined and fp_combined:
            ...
            effect_size = (
                abs(np.mean(tp_combined) - np.mean(fp_combined)) / pooled_std
                if pooled_std > 0
                else 0
            )
    ...
    plt.suptitle(
        f'Normalized + Combined Score Distributions - TP vs FP\n{genome_id or ""}',
        fontsize=14,
        y=1.00,
    )
    plt.tight_layout()
...
def compare_codon_usage(
    ...
    for codon, diff in codon_diffs[:10]:
        if diff > 0:
            print(
                f"  {codon}  +{diff:.6f}  (Coding: {codon_model.get(codon, 0):.6f}, "
